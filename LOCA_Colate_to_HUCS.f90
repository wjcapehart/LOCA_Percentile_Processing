...
program LOCA_Colate_to_HUCS
  ...
  integer (kind=2), dimension(nlon,nlat,nyear) :: map_pr, map_tasmax, map_tasmin

  real    (kind=4), dimension(nlon,nlat)  :: input_map
  ...
  netcdf_dims_3d_count = (/ nlon, nlat, 1 /)



  file_front_root = "/maelstrom2/LOCA_GRIDDED_ENSEMBLES/LOCA_NGP/"
 !SS file_front_root = "http://kyrill.ias.sdsmt.edu:8080/thredds/dodsC/LOCA_NGP/"
...
do s = 1, 1 ! nscen
  ...
  do e = 1, 1 ! nens
    ...
    filename_tasmax = trim(file_front_root)  // trim(scenarios(s)) // "/tasmin/NGP_LOCA_" // trim(tasmin_variable_name) // ".nc"
